...
async function testSalesImportAndRecommendations() {
  try {
    console.log('=== Starting Test: Sales Import and Price Recommendations ===');
    ...
    records.forEach((record: SalesHistoryItem) => {
    ...
    const salesByDate = new Map<string, SalesHistoryItem[]>();
    ...
    // Now run the price recommendations calculator
    console.log('Running price recommendations calculator...');
    
    const calculator = new PriceRecommendationCalculator(prisma);
    const recommendations = await calculator.getPriceRecommendations(
      365, // Use 1 year time range
      'low', // Use low confidence for testing
      user.id // Use our test user
    );
    
    console.log(`Generated ${recommendations.length} price recommendations`);
    
    if (recommendations.length > 0) {
      // Display the top 3 recommendations
      recommendations.slice(0, 3).forEach((rec, idx) => {
        console.log(`Recommendation ${idx+1}: ${rec.productName}, Current: $${rec.currentPrice.toFixed(2)}, Suggested: $${rec.recommendedPrice.toFixed(2)}, Impact: ${rec.percentageChange.toFixed(2)}%`);
      });
    } else {
      console.log('No recommendations found. This indicates a problem with product-sales relationships.');
      
      // Perform additional diagnostic
      console.log('Performing diagnostics...');
...
// Run the test
testSalesImportAndRecommendations()
  .then(() => process.exit(0))
  .catch(error => {
    console.error(error);
    process.exit(1);
